...
const hintButton = document.getElementById("getHint");   // 👈 button in HTML
const hintDisplay = document.getElementById("hintDisplay"); // 👈 container in HTML
const hintCounter = document.getElementById("hintCounter"); // 👈 counter in HTML
...
if (hintButton) {
    hintButton.addEventListener("click", async () => {
        const storedRiddle = localStorage.getItem("currentRiddle");
        if (!storedRiddle) {
            hintDisplay.innerText = "⚠️ No riddle loaded yet.";
            return;
        }

        const currentRiddle = JSON.parse(storedRiddle);
        const hint = await getHint(currentRiddle.id);
